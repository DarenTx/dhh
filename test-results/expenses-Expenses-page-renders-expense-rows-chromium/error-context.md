...
- Name: expenses.spec.ts >> Expenses page >> renders expense rows
- Location: e2e\expenses.spec.ts:123:7
...
Locator: getByText('HVAC filter replacement')
Expected: visible
Error: strict mode violation: getByText('HVAC filter replacement') resolved to 12 elements:
    1) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka getByText('HVAC filter replacement').first()
    2) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka getByText('HVAC filter replacement').nth(1)
    3) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka getByText('HVAC filter replacement').nth(2)
    4) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka getByText('HVAC filter replacement').nth(3)
    5) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka getByText('HVAC filter replacement').nth(4)
    6) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka getByText('HVAC filter replacement').nth(5)
    7) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka locator('div:nth-child(7) > .expense-left > .expense-desc')
    8) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka locator('div:nth-child(8) > .expense-left > .expense-desc')
    9) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka locator('div:nth-child(9) > .expense-left > .expense-desc')
    10) <span class="expense-desc" _ngcontent-ng-c3548373217="">HVAC filter replacement</span> aka locator('div:nth-child(10) > .expense-left > .expense-desc')
    ...

Call log:
  - Expect "toBeVisible" with timeout 5000ms
  - waiting for getByText('HVAC filter replacement')
...
```ts
  24  |       role: 'authenticated',
  25  |       email: 'manager@dahlheritagehomes.com',
  ...
  57  | }
  58  | 
  59  | async function mockExpensesData(page: Page) {
  60  |   await page.route('**/rest/v1/expenses**', (route) =>
  61  |     route.fulfill({
  62  |       status: 200,
  ...
  67  |           date: '2026-04-15',
  68  |           amount: 250.0,
  69  |           description: 'HVAC filter replacement',
  70  |           irs_category_id: 3,
  71  |           subcategory_id: 'sub-1',
  72  |           property_id: null,
  73  |           status: 'approved',
  74  |           is_active: true,
  75  |           created_by: 'manager-user-id',
  76  |           created_at: '2026-04-15T10:00:00Z',
  77  |           updated_at: '2026-04-15T10:00:00Z',
  78  |           irs_expense_categories: { id: 3, name: 'Cleaning and maintenance' },
  79  |           expense_subcategories: { id: 'sub-1', name: 'HVAC maintenance' },
  80  |           properties: null,
  81  |         },
  82  |         {
  83  |           id: 'exp-2',

...
- Name: guaranteed-payments.spec.ts >> Guaranteed Payments page >> shows the Log Hours button for managers
- Location: e2e\guaranteed-payments.spec.ts:107:7
...
Locator: getByRole('button', { name: /log hours/i })
Expected: visible
Timeout: 5000ms
...
```ts
  8   |       btoa(str).replace(/\+/g, '-').replace(/\//g, '_').replace(/=/g, '');
  9   |     const header = toBase64Url(JSON.stringify({ alg: 'HS256', typ: 'JWT' }));
  10  |     const payload = toBase64Url(
  11  |       JSON.stringify({
  12  |         sub: 'manager-user-id',
  13  |         aud: 'authenticated',
  14  |         exp: 9999999999,
  15  |         iat: 1700000000,
  16  |         role: 'authenticated',
  17  |         email: 'manager@dahlheritagehomes.com',
  18  |         app_metadata: { role: 'manager', is_active: true },
  19  |       }),
  20  |     );
  21  |     const mockUser = {

...
import {
  ...
  CreateGuaranteedPaymentPayload,
  GuaranteedPayment,
    ...
    <form class="form" [formGroup]="form" (ngSubmit)="submit()">
      <div class="row">
        ...
        <div class="field">
          ...
          />
          @if (
            form.get('hours_billed')?.touched && form.get('hours_billed')?.hasError('required')
          ) {
            <span class="error-msg">Hours are required.</span>
          }
          @if (form.get('hours_billed')?.touched && form.get('hours_billed')?.hasError('max')) {
            <span class="error-msg">Cannot exceed 16 hours in a single day.</span>
          }
        </div>
      </div>
      ...
      @if (monthlyHoursPreview() !== null) {
        <div class="hours-preview">
          Monthly total after this entry:
          <strong>{{ monthlyHoursPreview()!.toFixed(2) }} hrs</strong>
          @if (hourCap() !== null) {
            / {{ hourCap() }} hr cap
          }
        </div>
      ...
      <div class="field">
        ...
        @if (
          form.get('work_description')?.touched &&
          form.get('work_description')?.hasError('required')
        ) {
...
export class GuaranteedPaymentFormComponent implements OnInit {
  ...
  readonly form = new FormGroup({
    ...
    hours_billed: new FormControl<number | null>(null, [
      Validators.required,
      Validators.min(0.25),
      Validators.max(16),
    ]),
    ...
    this.gpService.createPayment(payload).subscribe({
      ...
      error: (err) => {
        ...
        this.serverError.set(
          err?.message ??
            'Failed to save entry. (A duplicate entry for this date may already exist.)',
        );
      },
    });
  }
